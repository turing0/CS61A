...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while place is not beehive:
            if self.min_range<=count<=self.max_range and len(place.bees):
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.index==0 and len(self.place.bees)>0:
            bees_win()
        if self.index!=0:
            ...
            p = self.place.exit
            while p:
                if p.ant and p.ant and not p.ant.buffed:
                    p.ant.damage *= 2
                    p.ant.buffed = True
                if isinstance(p.ant, ContainerAnt) and p.ant.contained_ant and not p.ant.contained_ant.buffed:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.direction==-1:
            if self.place.entrance is not Hive:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return (self.damage - self.insects_shot*0.05) - 0.2*distance
        # END Problem OPTIONAL
