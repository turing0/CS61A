...
def accuracy(typed, reference):
    ...
    for i in range(len(reference_words)):
        if i<n and typed_words[i] == reference_words[i]:
            count += 1
    return count/n*100
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for valid_word in valid_words:
        res.append(diff_function(user_word, valid_word, limit))
    idx = res.index(min(res))
    return valid_words[idx] if min(res)<=limit else user_word
    # END PROBLEM 5
...
def meowstake_matches(start, goal, limit):
    ...
    if limit==-1 or start=='' or goal=='': # Fill in the condition
        # BEGIN
        "*** YOUR CODE HERE ***"
        if limit==-1:
            return 1
        ...
        remove_diff = 1+meowstake_matches(start[1:],goal[1:],limit-1)  
        substitute_diff = 1+meowstake_matches(start[1:],goal,limit-1)
...
def fastest_words(game):
    ...
    words_lists = []
    for i in words:
        words_lists.append([time(game,p,i) for p in players])
    for k in words:
        idx = words_lists[k].index(min(words_lists[k]))
        words_lists[k] = [words_lists[k][idx], idx, k]    # [time, player, word_index]
    words_lists.sort(key=lambda x: x[1])
    # print(words_lists)
    while words_lists:
        idx = words_lists.index(min(words_lists, key=lambda x: x[0]))
        res[words_lists[idx][1]].append([words_lists[idx][2],word_at(game, words_lists[idx][2])])
        words_lists.pop(idx)
    for i in players:
        res[i].sort(key=lambda x: x[0])
        res[i] = [k[1] for k in res[i]]
    # print(words_lists)
    return res
    # END PROBLEM 10
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    #     return ret
    if user_word in word_cache:
        return word_cache[user_word]
    if user_word in valid_words:
        word_cache[user_word] = user_word
        return user_word
    ...
    for valid_word in valid_words:
        res.append(diff_function(user_word, valid_word, limit))
    idx = res.index(min(res))
    word_cache[user_word] = valid_words[idx] if min(res)<=limit else user_word
    return word_cache[user_word]
    # END PROBLEM EC2
