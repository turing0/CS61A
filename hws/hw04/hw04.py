...
def has_path(t, phrase):
    ...
    return any([has_path(b,phrase[1:]) for b in branches(t)]) if len(phrase)!=1 else True
...
def multiple_references_explanation():
    ...
    multiply may change the num's order
    ...
    see also: http://jots-jottings.blogspot.com/2011/09/sicp-exercise-214-broken-intervals.html
    """
...
def quadratic(x, a, b, c):
    ...
    p1=f(lower_bound(x))
    p2=f(upper_bound(x))
    if lower_bound(x)<piv and piv<upper_bound(x):
        p3=f(piv)
        return interval(p3, max(p1,p2)) if a>0 else interval(min(p1,p2), p3)
    else:
...
def check_par():
    ...
    True
    """
    r1 = interval(2, 8) # Replace this line!
