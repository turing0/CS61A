def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [[i, fn(i)] for i in seq if lower<=fn(i) and upper>=fn(i)]
...
def add_trees(t1, t2):
    ...
    b2s = branches(t2)
    tree_list = []
    lenght = min(len(b1s),len(b2s))
    for i in range(lenght):
        tree_list.append(add_trees(b1s[i],b2s[i]))
    tree_list += b1s[lenght:]
    tree_list += b2s[lenght:]

    return tree(label(t1)+label(t2), tree_list)
